...
```{r}
bike_orderlines_tbl %>% distinct(frame_material)
```
...
bike_orderlines_tbl %>%  
    ...
    rename(`Frame Material` = frame_material, Sales = sales) %>% 
...
bike_orderlines_tbl %>% 
    ...
    mutate(`Total Sales` = sum(Aluminum, Carbon)) %>% 
 ...
 #alternative for the scale dollar:
#mutate_if(is.numeric, scales::dollar)

#it is instead of replace na   
#pivot_wider(names_from = frame_material, values_from = total_sales, values_fill = 0) 
#spread(frame_material,value = total_sales,fill=0) 
